...
// TODO: Force the diploma to a 100 character limit
export const teachersRouter = createTRPCRouter({
  ...
  getAllWithRelations: protectedProcedure.query(async ({ ctx }) => {
    try {
      const teachers = await ctx.db.teacher.findMany({
        include: {
          ongoing: {
  ...
  getComment: protectedProcedure
    .input(z.object({ teacherId: z.string(), perimeterId: z.string() }))
    .query(async ({ input, ctx }) => {
      try {
        const comment = await ctx.db.teacherComment.findUnique({
          where: {
            teacherId_perimeterId: {
              teacherId: input.teacherId,
              perimeterId: input.perimeterId,
            },
          },
        });
        return comment;
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to fetch teacher comment",
        });
      }
    }),
  ...
  upsertComment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const comment = await ctx.db.teacherComment.upsert({
          where: {
            teacherId_perimeterId: {
              teacherId: input.teacherId,
              perimeterId: input.perimeterId,
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to save teacher comment",
        });
      }
    }),
});
