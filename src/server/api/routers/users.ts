  ...
  updateProfile: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { firstname, lastname, email, currentPassword, newPassword } =
        input;
      // A user can only edit their own profile: the target is taken from the
      // authenticated token, never from the client-supplied input.id, which
      // would otherwise allow editing anyone's profile.
      const id = ctx.userId;
