...
function TeacherLinkStep({
  ...
  submissionPhone: string;
  submissionDiplome: string;
  onLinked: (teacherId: string, teacherName: string) => void;
}) {
  ...
  const handleLink = async (teacher: TeacherResult) => {
    setError("");
    try {
      await linkMut.mutateAsync({ submissionId, teacherId: teacher.id });
      onLinked(teacher.id, `${teacher.firstname} ${teacher.lastname}`);
    } catch {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-slate-400">
          L&apos;enseignant sera ajouté comme intervenant potentiel sur ce
          module.
        </p>
      </div>
