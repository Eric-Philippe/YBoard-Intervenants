...
function specLabel(spec: string | null | undefined): string {
  // Legacy overrides for the old hardcoded survey categories; anything else
  // (free-text Promo.specialty from the dynamic per-perimeter survey) is
  // sentence-cased so "DATA ENGINEER" reads as "Data engineer".
  ...
  if (!spec) return "";
  return map[spec] ?? capitalize(spec);
}
        ...
        <div className="mt-1 ml-3 pl-3 border-l-2 border-blue-200 space-y-px">
          {subjects.map((subj) => (
            ...
            >
              ...
              <span className="leading-snug">{capitalize(subj.title)}</span>
            </button>
          ))}
        </div>
      )}
...
function YearGroup({
  ...
  if (categories.length === 1) {
    ...
    const label = cat.specialization ? `${yearTitle} - ${specLabel(cat.specialization)}` : cat.label;
    return (
      <NavItem
        label={label}
