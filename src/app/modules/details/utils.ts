...
type PrismaTeacherRelation = {
  ...
  workload: number;
  rate?: Decimal | null;
  rateTDP?: Decimal | null;
  rateFFP?: Decimal | null;
  teacher: {
    id: string;
    lastname: string;
    firstname: string;
...
export const transformPrismaRelation = (
  ...
  return {
    ...
    rateTDP: relation.rateTDP ? extractNumericRate(relation.rateTDP) : null,
    rateFFP: relation.rateFFP ? extractNumericRate(relation.rateFFP) : null,
    teacher: {
...
export const calculateWorkloadStats = (module: PromoModule): WorkloadStats => {
  ...
  const calculateCostAndRate = (
    ...
    relations.forEach((relation) => {
      const teacherRate = relation.teacher?.rate
        ? extractNumericRate(relation.teacher.rate)
        : 0;
      const { cost, mode } = calculateAssignationCost({
        workload: relation.workload ?? 0,
        rate: relation.rate,
        rateTDP: relation.rateTDP,
        rateFFP: relation.rateFFP,
        teacherRate,
        moduleNombreHeureTDP: module.module.nombreHeureTDP,
        moduleNombreHeureFFP: module.module.nombreHeureFFP,
      });

      totalCost += cost;

      const rate =
        mode === "standard" ? (relation.rate ?? teacherRate) : 0;
      if (rate > 0) {
        totalRateSum += rate;
        validRates++;
      }
