...
const TABS = [
  ...
  { id: "submissions", label: "Réponses", icon: FaList },
  { id: "subjects", label: "Par matière", icon: FaBook },
...
export default function SondageAdminPage() {
  ...
  const { data: configData } = api.survey.getConfig.useQuery();
  const categories = (configData?.categories ?? []) as SurveyCategory[];

  const baseUrl = typeof window !== "undefined" ? window.location.origin : "";
  const surveyUrl = `${baseUrl}/sondage`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-screen-xl px-4 py-6 sm:px-6">
        ...
        {tab === "submissions" && <SubmissionList categories={categories} />}

        {tab === "subjects" && <SubjectView categories={categories} />}
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mb-4 text-sm text-slate-500">
                ...
                La source UTM est enregistrée avec chaque réponse.
              </p>
              <div className="space-y-3">
                {[
                  ...
                  return (
                    ...
                    >
                      <code className="flex-1 truncate text-sm text-blue-700">
                        {url}
                      </code>
                      <button
                        onClick={() => void navigator.clipboard.writeText(url)}
                        className="flex-shrink-0 rounded-md border border-gray-200 bg-white p-1.5 text-slate-400 transition-colors hover:text-blue-600"
                        title="Copier"
                      >
