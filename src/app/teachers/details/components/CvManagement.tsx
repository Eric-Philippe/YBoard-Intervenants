...
export function CvManagement({ teacher, onCvUpdate }: CvManagementProps) {
  ...
  const handleFileUpload = async (
    ...
    if (file.type !== "application/pdf") {
      notifications.show({ message: "Seuls les fichiers PDF sont acceptés", color: "orange" });
      return;
    }

    // Validate file size (5MB max)
    if (file.size > 5 * 1024 * 1024) {
      notifications.show({ message: "La taille du fichier ne doit pas dépasser 5 Mo", color: "orange" });
      return;
    }
  ...
  const handleDeleteCv = async () => {
    ...
    try {
      ...
      notifications.show({ title: "Erreur", message: "Erreur lors de la suppression du CV", color: "red" });
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
      ...
      {teacher.cv_filename ? (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleViewCv} className="btn-glass">
              <svg
            ...
            <button onClick={() => setConfirmDeleteOpen(true)} disabled={deleting} className="btn-danger">
              {deleting ? (
                <svg
    ...
    >
      <div className="p-6">
        <h3 className="mb-2 text-base font-semibold text-gray-900">Supprimer le CV</h3>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => { setConfirmDeleteOpen(false); void handleDeleteCv(); }}
