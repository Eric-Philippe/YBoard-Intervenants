...
// localStorage keys are scoped per perimeter slug (see SurveyProvider) so that a
// draft saved for one perimeter never leaks its subject identifiers into another,
// which would produce stale references at submission time.
const lsFormKey = (slug: string) => `survey_form_data_${slug}`;
const lsResponsesKey = (slug: string) => `survey_subject_responses_${slug}`;
const lsSectionKey = (slug: string) => `survey_current_section_${slug}`;
...
export function SurveyProvider({
  ...
  const LS_FORM = lsFormKey(perimeterSlug);
  const LS_RESPONSES = lsResponsesKey(perimeterSlug);
  const LS_SECTION = lsSectionKey(perimeterSlug);

  const [categories, setCategories] = useState<SurveyCategory[]>([]);
  const [perimeterTitle, setPerimeterTitle] = useState("");
  ...
  const configQuery = api.survey.getConfig.useQuery(
    { slug: perimeterSlug },
    { retry: false },
  );
  ...
  useEffect(() => {
    if (configQuery.data) {
      ...
      setPerimeterTitle(configQuery.data.perimeter.title);
      setLoading(false);
    }
  ...
  }, [formData.niveauAcademique, currentSection, categories, LS_SECTION]);
